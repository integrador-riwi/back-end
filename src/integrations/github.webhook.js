...
router.post('/github', verifySignature, async (req, res) => {
  ...
  if (event === 'organization' && action === 'member_removed') {
    ...
    try {
      ...
      for (const row of teams.rows) {
        ...
        console.log(`[GitHub Webhook] Team ${row.id_team} → ${JSON.stringify(result)}`);
      }
    } catch (error) {
      console.error('[GitHub Webhook] Error on org member_removed:', error.message);
    }
    return;
  }
  ...
  if (event === 'member' && action === 'removed') {
    ...
    try {
      ...
      if (!teamProject) {
        console.warn(`[GitHub Webhook] Repo ${repoName} not linked to any team`);
        return;
      }

      const result = await removeMemberFromTeam(teamProject.id_team, user.id_user);
      console.log(`[GitHub Webhook] Team ${teamProject.id_team} → ${JSON.stringify(result)}`);
    } catch (error) {
      console.error('[GitHub Webhook] Error on repo member removed:', error.message);
    }
    return;
  }
