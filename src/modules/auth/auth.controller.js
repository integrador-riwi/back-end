...
export const register = asyncHandler(async (req, res) => {
  const { name, email, password, role, documentNumber, documentType, clan } =
    req.body;
...
export const githubAuth = asyncHandler(async (req, res) => {
  const userId = req.user.id_user;
  const authUrl = await AuthService.getGithubAuthUrl(userId);
...
export const githubCallback = asyncHandler(async (req, res) => {
  ...
  if (githubError) {
    return res.redirect(
      `${config.client.url}/settings/github?error=${encodeURIComponent(githubError)}`,
    );
  }

  if (!code) {
    return res.redirect(`${config.client.url}/settings/github?error=no_code`);
  }
  ...
  try {
    if (state) {
      ...
      if (Date.now() - decoded.ts > tenMinutes) {
        return res.redirect(
          `${config.client.url}/settings/github?error=state_expired`,
        );
      }
    }
  } catch {
    return res.redirect(
      `${config.client.url}/settings/github?error=invalid_state`,
    );
  }

  if (!userId) {
    return res.redirect(
      `${config.client.url}/settings/github?error=not_authenticated`,
    );
  }
