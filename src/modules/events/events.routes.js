...
const canManage = hasRole("ADMIN", "TL_DEVELOPMENT", "TL_SOFT_SKILLS", "TL_ENGLISH");
...
router.post("/:id/rubrics", authenticate, canManage, EventsController.addRubrics);

// PUT  /api/events/:id/rubrics/:rubricId — update a single rubric
router.put("/:id/rubrics/:rubricId", authenticate, canManage, EventsController.updateRubric);

// DELETE /api/events/:id/rubrics/:rubricId — soft-delete a rubric
router.delete("/:id/rubrics/:rubricId", authenticate, isAdmin, EventsController.deleteRubric);

// Ranking
import RankingController from "./ranking.controller.js";

// GET  /api/events/:id/ranking-status
router.get("/:id/ranking-status", authenticate, isAdmin, RankingController.getRankingStatus);

// POST /api/events/:id/publish-ranking
router.post("/:id/publish-ranking", authenticate, isAdmin, RankingController.publishRanking);
