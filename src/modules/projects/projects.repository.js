...
export const findAll = async ({ search = null, page = 1, limit = 10 }) => {
  ...
  if (search) {
    whereClauses.push(`p.name ILIKE $${paramIndex++} OR t.name ILIKE $${paramIndex++}`);
...
export const updateDeliverables = async (id, { videoUrl, presentationUrl, previewPhotoUrl }) => {
  const query = `
    UPDATE projects
    SET 
  ...
  const result = await pool.query(query, [videoUrl, presentationUrl, previewPhotoUrl, id]);
  return result.rows[0] || null;
};
